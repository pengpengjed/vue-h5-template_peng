<template>
  <van-cell :title="title" class="cs-data-cell" :class="{ editable: editable, clearable: clearable }"
    :is-link="!disabled" @click="onCellClick" :required="required">
    ...
    <div v-else :class="{ 'no-data': !hasSelectedValue }">
      ...
      <div class="picker-single-value" v-else>
        <input type="text" class="input-text" v-if="editable && !disabled" :value="componentValue ? componentText : ''"
          @click.stop="onInputClick" @change="onInputKeyUp" placeholder="请选择或者输入" />
    ...
    <van-popup v-model="popupVisible" position="bottom" get-container="body" v-if="!disabled" @click-overlay="onCancel">
      <!--单选-->
      <template v-if="!multiple">
        <van-picker :columns="pickerColumns" @confirm="onConfirm" @cancel="onCancel" :default-index="pickerValue"
          show-toolbar :title="pickerTitle" ref="picker">
          <input class="picker-search" v-model="keyword" slot="title" type="text" placeholder="请输入关键字搜索"
            v-if="searchable" />
        </van-picker>
      </template>
      <!--多选-->
      <template v-else>
        <div class="cs-data-popup-multiple">
          <div class="header">
            ...
            <div class="title">
              <input class="picker-search" v-model="keyword" type="text" placeholder="请输入关键字搜索" v-if="searchable" />
          ...
          <div class="content">
            <van-checkbox-group v-model="pickerValue">
              <van-cell-group :key="cellKey">
                <van-cell @click="toggle(pickerIndex)" v-for="(item,pickerIndex) in pickerColumns" clickable
                  :key="item[valueField]" :title="item[textField]">
                  <template #right-icon>
